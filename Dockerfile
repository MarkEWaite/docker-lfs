...
RUN apt-get update && apt-get upgrade -y && apt-get install -y git curl ca-certificates locales lsb-release make patch rsync && curl -s https://packagecloud.io/install/repositories/github/git-lfs/script.deb.sh | bash && apt-get install -y git-lfs && git lfs install && rm -rf /var/lib/apt/lists/*
